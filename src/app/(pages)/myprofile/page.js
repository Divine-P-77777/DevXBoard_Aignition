  ...
  const handleProfileSubmit = async () => {
    ...
    try {
      const res = await fetch(`/api/profile/${user.id}`, {
        ...
        body: JSON.stringify({ username: username.trim(), pic: imageUrl }),
      });
      const json = await res.json();
...
const handlePicUpload = async (uploadedUrl) => {
  try {
    setImageUrl(uploadedUrl);

    // Update pic in profile API
    const res = await fetch(`/api/profile/${user.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (res.ok) {
      ...
      setMessage(data.error || 'Failed to update profile picture');
    }
  } catch (err) {
  ...
  return (
    <div className={`min-h-[1000px] pt-30 px-4 ${isDarkMode ? 'bg-black' : 'bg-gradient-to-br from-pink-50 to-purple-100'}`}>
