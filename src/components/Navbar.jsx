...
const Navbar = () => {
  ...
  const glassStyle = isDark
    ? "bg-black/30 border border-pink-500 text-white"
    : "bg-white/70 border border-pink-400 text-black";

  const navBtnStyle = (path) => {
    const isActive = path === router.pathname;
    return `px-4 py-2 rounded-full transition-all duration-300 text-sm font-medium ${isActive
      ? "bg-purple-600 text-white shadow-lg"
      : isDark
        ? "hover:bg-pink-500/20 text-white"
  ...
  return (
    <nav className={`fixed top-4 left-1/2 transform -translate-x-1/2 z-40 w-[92%] px-4 py-3 rounded-full shadow-md backdrop-blur-2xl transition-all duration-300 ${glassStyle} ${scrolled ? "shadow-lg" : ""}`}>
      <div className="flex justify-between items-center max-w-7xl mx-auto">
        <Link href="/" className="text-lg font-bold tracking-tight flex items-center gap-2">
          <img src="/favicon.png" className="w-10 h-10 rounded-full" alt="Logo" />
          <span>DevXBoard</span>
        </Link>
          ...
          {profile ? (
            <Link href="/myprofile" title="My Profile">
              <img
                ...
                className="w-8 h-8 rounded-full object-cover border border-pink-300 hover:scale-105 transition"
                alt="Profile"
              />
            </Link>
          ) : user ? (
            <Link href="/myprofile" title="My Profile">
              <User size={20} className="text-pink-400 hover:text-pink-600" />
            </Link>
          ) : (
            <Link href="/auth" className="text-pink-600 hover:underline text-sm">Login</Link>
          )}
        </div>
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            ...
            className={`fixed top-0 left-[-8px] z-50 p-6 pt-20 w-64
              ${isDark ? "bg-black" : "bg-white"} backdrop-blur-2xl
              border-r ${isDark ? "border-pink-500" : "border-pink-400"}
              ${isDark ? "text-white" : "text-black"} shadow-xl rounded-2xl`}
          >
            <ul className="flex flex-col gap-4 text-base">
              ...
              <li>
                {profile ? (
                  <Link href="/myprofile" onClick={() => setMenuOpen(false)} className="flex items-center gap-2">
                    <img
                      src={profile.pic || "/default-avatar.png"}
                  ...
                  <Link href="/auth" onClick={() => setMenuOpen(false)} className="text-pink-600 hover:underline">Login</Link>
                )}
              </li>
            </ul>
          </motion.div>
      ...
      {showLogoutPopup && (
        <div className="fixed top-80 inset-0 z-[100] flex items-center justify-center bg-black/40">
          <ConfirmPopup
            visible={true}
